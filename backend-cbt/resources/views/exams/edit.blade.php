<x-layouts.app :title="'Edit Paket Ujian - ' . $exam->name">
    <div class="max-w-4xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Paket Ujian: {{ $exam->name }}</h1>
                <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Perbarui konfigurasi durasi, pengacakan, dan butir soal.</p>
            </div>
            <a href="{{ route('exams.show', $exam) }}" class="px-3.5 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs border border-slate-200">
                Detail Ujian
            </a>
        </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $exam->duration_minutes) }}" min="1" max="300" required
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="description" rows="3"
                                    class="w-full p-3.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-brand-700 outline-none transition">{{ old('description', $exam->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <label class="flex items-center p-3.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition">
                                <input type="checkbox" name="randomize_questions" value="1" {{ old('randomize_questions', $exam->randomize_questions) ? 'checked' : '' }} class="h-4 w-4 text-brand-700 focus:ring-brand-500 rounded">
                                <span class="ml-3 text-xs font-semibold text-slate-800">Acak Urutan Butir Soal</span>
                            </label>
                            <label class="flex items-center p-3.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition">
                                <input type="checkbox" name="randomize_options" value="1" {{ old('randomize_options', $exam->randomize_options) ? 'checked' : '' }} class="h-4 w-4 text-brand-700 focus:ring-brand-500 rounded">
                                <span class="ml-3 text-xs font-semibold text-slate-800">Acak Urutan Pilihan Opsi</span>
                            </label>
                            <label class="flex items-center p-3.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition">
                                <input type="checkbox" name="allow_back" value="1" {{ old('allow_back', $exam->allow_back) ? 'checked' : '' }} class="h-4 w-4 text-brand-700 focus:ring-brand-500 rounded">
                                <span class="ml-3 text-xs font-semibold text-slate-800">Boleh Navigasi Bolak-balik</span>
                            </label>
                            <label class="flex items-center p-3.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition">
                                <input type="checkbox" name="show_result_after" value="1" {{ old('show_result_after', $exam->show_result_after) ? 'checked' : '' }} class="h-4 w-4 text-brand-700 focus:ring-brand-500 rounded">
                        ...
                        <div class="max-h-96 overflow-y-auto border border-slate-200 rounded-xl divide-y divide-slate-100">
                            ...
                            <label class="p-3.5 flex items-center hover:bg-slate-50 transition cursor-pointer">
                                <input type="checkbox" name="question_ids[]" value="{{ $question->id }}" 
                                    ...
                                    class="h-4 w-4 text-brand-700 focus:ring-brand-500 rounded">
