<x-layouts.app :title="'Edit Kelas - ' . $class->name">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Kelas: {{ $class->name }}</h1>
                <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Perbarui informasi rombongan belajar dan tahun ajaran.</p>
            </div>
            <a href="{{ route('classes.show', $class) }}" class="px-3.5 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs border border-slate-200">
                Detail Siswa
            </a>
        </div>
                ...
                <div class="p-6 sm:p-8 space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="grade" value="{{ old('grade', $class->grade) }}"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                            class="w-full p-3.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-brand-700 outline-none transition">{{ old('description', $class->description) }}</textarea>
                    </div>
                    ...
                    <div class="pt-2">
                        <label class="flex items-center p-3.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 transition">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $class->is_active) ? 'checked' : '' }} class="h-4 w-4 text-brand-700 rounded">
