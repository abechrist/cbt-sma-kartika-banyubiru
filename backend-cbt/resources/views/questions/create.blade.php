<x-layouts.app :title="'Tambah Soal Baru'">
    <div class="max-w-4xl mx-auto">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Tambah Butir Soal Baru</h1>
                <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Lengkapi formulir di bawah ini untuk menambahkan soal ke bank soal sekolah.</p>
            </div>
        </div>
                ...
                <div class="p-6 sm:p-8 space-y-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <select name="class_id"
                                    class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-800 font-medium focus:bg-white focus:ring-2 focus:ring-brand-700 outline-none transition">
                                    <option value="">Pilih Kelas (Opsional - Terbuka Umum)</option>
                            ...
                            <div>
                                ...
                                <select name="type" required id="question-type"
                                    ...
                                    @foreach($types as $key => $label)
                                        <option value="{{ $key }}" {{ old('type', 'pg') == $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" min="0" name="score" value="{{ old('score', 1) }}" required
                            ...
                            <div>
                                ...
                                <input type="text" name="competency_code" value="{{ old('competency_code') }}" placeholder="Contoh: KD 3.1 atau CP-01"
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="question_text" rows="4" required placeholder="Tuliskan pertanyaan soal di sini..."
                                    class="w-full p-4 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 focus:bg-white focus:ring-2 focus:ring-brand-700 outline-none leading-relaxed transition">{{ old('question_text') }}</textarea>
                            </div>
                        </div>
                    </div>

                    <!-- Options for PG & Matching -->
                    <div id="options-container">
                        ...
                        <div id="options-wrapper" class="space-y-3">
                            ...
                            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 bg-slate-50 border border-slate-200 rounded-xl option-row">
                                <input type="text" name="options[{{ $i }}][label]" placeholder="A/B/C" value="{{ chr(65 + $i) }}" required
                                ...
                                <input type="text" name="options[{{ $i }}][option_text]" placeholder="Tulis teks opsi jawaban..." required
                                ...
                                <label class="flex items-center is-correct-wrap px-3 py-1.5 bg-white border border-slate-300 rounded-lg cursor-pointer hover:bg-emerald-50 transition">
                                    <input type="checkbox" name="options[{{ $i }}][is_correct]" value="1" {{ $i === 0 ? 'checked' : '' }} class="h-4 w-4 text-emerald-600 focus:ring-emerald-500 rounded">
                                    <span class="ml-2 text-xs font-bold text-slate-700">Kunci Benar</span>
                                </label>
                                <input type="text" name="options[{{ $i }}][correct_match]" placeholder="Pasangan cocok (cth: Definisi A)"
                                    class="flex-1 px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-brand-700 outline-none correct-match-input hidden">
                                <input type="number" name="options[{{ $i }}][sort_order]" value="{{ $i }}" title="Urutan" class="w-16 px-3 py-2 bg-white border border-slate-300 rounded-lg text-center text-xs">
                    ...
                    <div>
                        <h2 class="text-sm font-bold uppercase tracking-wider text-slate-700 pb-3 border-b border-slate-200 mb-5 flex items-center gap-2">
                            ...
                            Media Pendukung Soal (Opsional)
                        </h2>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl">
                                <label class="block text-xs font-bold text-slate-700 uppercase mb-2">Lampiran Gambar</label>
                            ...
                            <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl">
                                <label class="block text-xs font-bold text-slate-700 uppercase mb-2">File Audio (Listening)</label>
                            ...
                            <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl">
                                <label class="block text-xs font-bold text-slate-700 uppercase mb-2">File Video</label>
    ...
    <script>
        ...
        function addOption() {
            ...
            if (count >= 10) {
                alert('Maksimal 10 opsi jawaban');
                return;
            }
            ...
            div.innerHTML = `
                <input type="text" name="options[${count}][label]" value="${labelLetter}" placeholder="Label" required
                    class="w-full sm:w-16 px-3 py-2 bg-white border border-slate-300 rounded-lg text-center font-bold text-sm text-slate-800 focus:ring-2 focus:ring-brand-700 outline-none">
                <input type="text" name="options[${count}][option_text]" placeholder="Tulis teks opsi..." required
                    class="w-full flex-1 px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm text-slate-800 focus:ring-2 focus:ring-brand-700 outline-none">
                <label class="flex items-center is-correct-wrap px-3 py-1.5 bg-white border border-slate-300 rounded-lg cursor-pointer hover:bg-emerald-50 transition">
                    <input type="checkbox" name="options[${count}][is_correct]" value="1" class="h-4 w-4 text-emerald-600 rounded">
                    <span class="ml-2 text-xs font-bold text-slate-700">Kunci Benar</span>
                </label>
                <input type="text" name="options[${count}][correct_match]" placeholder="Pasangan cocok (cth: Definisi)"
